...
if( ! class_exists('CreativeCommons') ) {
  class CreativeCommons {
    ...
    function __construct() {
      ...
      $mo = plugin_dir_path(dirname(__FILE__))
...
function wphub_register_options_page() {
    ...
    public function page_init() {
      ...
      register_setting( 'cc-admin', 'license', array(&$this, '_wrapper_settings_api_verify') );
      add_settings_section( 'license-section', '', array(&$this, 'settings_license_section'), 'cc-admin');

      add_settings_field( 'license_current', '<label for="license-current">' . __('Current default license', $this->localization_domain) . '</label>', array(&$this, 'setting_license_default_field'), 'cc-admin', 'license-section');

      add_settings_field( 'license', '', array(&$this, 'setting_license_field'), 'cc-admin', 'license-section');

      add_settings_field( 'warning_txt', '<label for="warning_txt">' . __('Add license warning text', $this->localization_domain) . '</label>', array(&$this, 'setting_warning_field'), 'cc-admin', 'license-section');

      add_settings_field( 'attribution_to', '<label for="attribution_to">' . __('Set attribution to', $this->localization_domain) . '</label>', array(&$this, 'setting_attribution_field'), 'cc-admin', 'license-section');
      add_settings_field( 'allow_user_override', '<label for="allow_user_override">' . __('Allow users to override site-wide license', $this->localization_domain) . '</label>', array(&$this, 'setting_user_override_license_field'), 'cc-admin', 'license-section');
      add_settings_field( 'allow_content_override', '<label for="allow_content_override">' . __('Allow a different license per post/page', $this->localization_domain) . '</label>', array(&$this, 'setting_content_override_license_field'), 'cc-admin', 'license-section');
    ...
    public function print_license_html( $location = 'frontend', $echo = true) {
      ...
      if( is_array($license) && sizeof($license) > 0 ) {
        ...
        $html = "<div class='license-wrap'>"
              . $this->html_rdfa($deed_url, $license_name, $image_url,
                                 $title_work, is_singular(),
                                 $attribute_url, $attribute_text,
                                 false, false, $warning_text)
              . '</div>';
      }
      if( $echo ) {
        echo $html;
    ...
    public function html_rdfa($deed_url, $license_name, $image_url, $title_work,
                              $is_singular, $attribute_url, $attribute_text,
                              $source_work_url, $extra_permissions_url,
                              $warning_text) {
      $html = '';
  ...
  $license = new CreativeCommons();
 } else {
  $this->_logger('Could not instantiate class CreativeCommons due to already existing class CreativeCommons.');
 }
